...
// (the number of complete layers) that we can build, given 'n' number of cubes as the 
// argument.
...
// orner cube below it and so each row has one more cube than the one below it.
// If you were given 14 cubes, you could build a pyramid of 3 layers, but 13 wouldn't be 
// enough as you would be missing one cube, so only 2 layers would be complete and some 
// cubes left over!
...
var pyramidHeight = function(n){
  let layers = 0;
  while (true) {
    if (n - layers * layers < (layers + 1) * (layers + 1)) return layers;
    n -= layers * layers;
    layers++;
  };
}
